    ...
    var body: some View {
        ...
        .task {
            while !Task.isCancelled {
                ...
                if credit != nil {
                    // Credit is the expensive part; hold the line when set.
                    try? await Task.sleep(nanoseconds: 60_000_000_000)
                } else {
                    // No data yet (agent still starting or backend busy);
                    // retry quickly so 0% becomes the real value promptly.
        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let cbc = session.cbcSessionID, !cbc.isEmpty {
                    ...
                    ContextUsageView(agent: currentAgent, cbcSessionID: detectedCBC, projectDir: session.projectDir)
                } else if let detectedAgent {
                    ContextUsageView(agent: detectedAgent, cbcSessionID: nil, projectDir: session.projectDir)
                    ...
                    ContextUsageView(agent: currentAgent, cbcSessionID: nil, projectDir: session.projectDir)
                }
