...
Claude Code，在侧边栏查看每个对话的**上下文窗口占用**，并把对话**精确
恢复到离开时的位置**——换一台 Mac 也一样。

## 为什么选 lmux？

- **一等公民的 CodeBuddy Code 支持** —— 同类工具中唯一基于 CodeBuddy 会话存储构建
  的 GUI：自动发现对话、一键续接、**精确**的上下文窗口计量（Claude Code
  同样支持，用量为估算值）。
...
- **会话续接** —— 会话记住自己的智能体对话，重连时自动恢复；导入的对话会做
  路径本地化，在新机器上同样能续接。
- **上下文计量** —— 侧边栏显示每个对话的上下文窗口占用（CodeBuddy 精确、
  Claude 估算），便于在会话爆掉之前先 /compact。
- **等待输入检测** —— 智能体答完一轮停下来等你时，侧边栏高亮并推送通知，
  并行跑多个会话时一眼看出哪个在等你；直接读会话记录判断，无需配置 hooks。
...
**和 tmux 加几个终端窗口有什么区别？**
会话↔对话绑定、自动续接、按对话的上下文计量、跨 Mac 同步——这些是一个
纯终端复用器都不具备的。
...
lmux-app/
  Sources/
    ...
    LMUXCoreTests/ # 单元测试
  backend-src/     # Go 后端：会话存储（SQLite）、智能体扫描、上下文统计、
                   # REST API（端口 19680）
  tools/
